...
export const errorMessages: Record<string, AppError> = {
  docker_unavailable: {
    ...
    suggestion:
      "Install Docker Desktop and make sure it's running. You can download it from docker.com.",
  },
  ...
  installation_failed: {
    ...
    suggestion:
      "Check that you have enough disk space and admin permissions, then try again.",
  },
  config_error: {
    message: "There's a problem with the configuration file.",
    suggestion:
      "The config file may be corrupted. Try resetting it from Settings, or check the file for syntax errors.",
  },
  unsupported_platform: {
    message: "Your operating system is not supported.",
    suggestion:
      "ClawStation supports Windows 10+ and Linux. macOS support is coming soon.",
  },
  network_error: {
    message: "Could not connect to the internet.",
    suggestion:
      "Check your network connection and try again. A firewall or proxy may be blocking access.",
  },
  openclaw_not_running: {
    message: "OpenClaw is not running.",
    suggestion:
      "Start OpenClaw from the Install page or via Docker: docker run openclaw",
  },
  api_unavailable: {
    message: "OpenClaw API is not responding.",
    suggestion:
      "OpenClaw may be starting up. Wait a moment and try refreshing.",
  },
  permission_denied: {
  ...
  uninstall_failed: {
    ...
    suggestion:
      "Some containers or files may still exist. Try removing them manually via Docker CLI or file manager.",
  },
  update_failed: {
    message: "OpenClaw update failed.",
    suggestion:
      "Check your internet connection and try again. For native installs, run: npm install -g openclaw@latest",
  },
  version_check_failed: {
    message: "Could not check for updates.",
    suggestion:
      "Check your internet connection. The GitHub releases API may be temporarily unavailable.",
  },
  unknown: {
    message:
      "Something went wrong. Try again, or check the details below for a fix.",
    suggestion:
      "An unexpected error occurred. Restart the app. If this keeps happening, check the logs.",
  },
};
...
export function formatError(error: unknown): AppError {
  ...
  if (error instanceof Error) {
    ...
    return {
      ...
      suggestion: errorMessages.unknown.suggestion,
    };
  }
...
function matchErrorPattern(message: string): AppError | null {
  ...
  if (lower.includes("docker") && lower.includes("not installed"))
    return errorMessages.docker_not_installed;
  if (lower.includes("docker") && lower.includes("daemon"))
    return errorMessages.docker_daemon_not_running;
  if (lower.includes("docker desktop"))
    return errorMessages.docker_desktop_not_running;
  if (lower.includes("wsl") || lower.includes("wsl2"))
    return errorMessages.wsl_backend_not_ready;
  if (lower.includes("openclaw") && lower.includes("not running"))
    return errorMessages.openclaw_not_running;
  if (lower.includes("openclaw") && lower.includes("api"))
    return errorMessages.api_unavailable;
  if (
    lower.includes("update") &&
    (lower.includes("failed") || lower.includes("pull"))
  )
    return errorMessages.update_failed;
  if (lower.includes("version check") || lower.includes("releases"))
    return errorMessages.version_check_failed;
  ...
  if (lower.includes("install") && lower.includes("uninstall"))
    return errorMessages.uninstall_failed;
